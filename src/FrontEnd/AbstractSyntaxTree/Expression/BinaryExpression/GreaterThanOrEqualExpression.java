...
import BackEnd.ControlFlowGraph.Instruction.ArithmeticInstruction.BinaryInstruction.GreaterThanOrEqualInstruction;
...
public class GreaterThanOrEqualExpression extends BinaryExpression {
	private GreaterThanOrEqualExpression(Type type, boolean isLeftValue, Expression left, Expression right) {
	...
	public static Expression getExpression(Expression left, Expression right) {
		if (left.type instanceof IntType && right.type instanceof IntType) {
			...
			return new GreaterThanOrEqualExpression(BoolType.getType(), false, left, right);
	...
	public void emit(List<Instruction> instructions) {
		...
		instructions.add(GreaterThanOrEqualInstruction.getInstruction(operand, left.operand, right.operand));
	}
}
